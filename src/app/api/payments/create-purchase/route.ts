...
export async function POST(req: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${siteUrl || process.env.NEXT_PUBLIC_SITE_URL}/exchange?success=true&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${siteUrl || process.env.NEXT_PUBLIC_SITE_URL}/exchange?canceled=true`,
      metadata: {
        user_id: user_id,
