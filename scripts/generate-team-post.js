...
async function generateTeamPost() {
  try {
    console.log('🚀 Starting team post generation...');
    ...
    console.log(`✅ Team post created successfully! Post ID: ${newPost.id}`);
    console.log(`📅 Timestamp: ${new Date().toISOString()}`);

    return newPost;

  } catch (error) {
    console.error('💥 Error generating team post:', error);
    throw error;
  }
}
...
if (require.main === module) {
  generateTeamPost()
    .then(() => {
      console.log('🎉 Team post generation completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('💥 Team post generation failed:', error);
      process.exit(1);
    });
}
